...
const TagCard = ({ tag, title, readTime, isWhite, id, disabled }: TagCardProps) => {
  return (
    <div className="absolute bottom-8 lg:bottom-14 md:left-10 left-1/2 transform -translate-x-1/2 md:translate-x-0 z-40">
      <div className={` max-w-[220px] p-2 lg:p-4 ${isWhite ? 'bg-white' : 'bg-black'}`}>
        <div className="bg-[#D8F34E] rounded-full lg:p-2 p-1">
          <p className="text-black text-[12px] lg:text-sm text-center font-bold">{tag}</p>
        </div>
      </div>
      <div className={` min-w-[250px] lg:max-w-[440px] p-2 lg:p-4  ${isWhite ? 'bg-white' : 'bg-black'}`}>
        <p className={` max-w-[480px] text-base lg:text-4xl font-bold leading-normal ${isWhite ? 'text-black' : 'text-white'}`}>
          {title}
        </p>
        <div className="flex items-center justify-between gap-2 mt-2 lg:mt-8">
          <Link className={`${disabled ? 'hidden' : ''}`} href={`/posts/${id}`}>
            <Button endContent={<RigthArrowIcon color="#9C73F7" width={44} height={44} />} className="bg-transparent">
              <p className={`text-base lg:text-3xl font-bold ${isWhite ? 'text-black' : 'text-white'}`}>Read</p>
            </Button>
          </Link>
